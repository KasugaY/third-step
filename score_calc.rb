...
printf "理科の合計点:%.2f（点）	英語の合計点:%.2f（点）\n", sienceArray.sum, englishArray.sum

#偏差値表示のプログラム
(1..19).each{|i|
	printf  "%d人目の理科の偏差値:%.3f	英語の偏差値:%.3f\n", i,  50 + 10 * ((sienceArray[i] - sienceArray.mean) / sienceArray.stddev), 50 + 10 * ((englishArray[i] - englishArray.mean) / englishArray.stddev)
}

#ソートのプログラム(降順)
printf "理科の高得点順\n"
p sienceArray.sort.reverse.to_a
printf "\n"
printf "英語の高得点順\n"
